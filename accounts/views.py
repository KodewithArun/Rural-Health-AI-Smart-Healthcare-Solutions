...
from .forms import VillagerRegistrationForm, HealthWorkerCreationForm, UserProfileUpdateForm, HealthWorkerProfileUpdateForm, CustomPasswordChangeForm
    ...
    if request.method == 'POST':
        ...
        if 'update_profile' in request.POST:
            ...
            if user.is_health_worker and health_profile:
                health_form = HealthWorkerProfileUpdateForm(request.POST, instance=health_profile)
...
@login_required
@user_passes_test(lambda u: u.is_authenticated and u.is_health_worker, login_url='accounts:login')
def health_worker_dashboard(request):
    ...
    pending_appointments = Appointment.objects.filter(healthworker=user, status='pending').count()
    approved_appointments = Appointment.objects.filter(healthworker=user, status='approved').count()
    completed_appointments = Appointment.objects.filter(healthworker=user, status='completed').count()
    cancelled_appointments = Appointment.objects.filter(healthworker=user, status='cancelled').count()
    ...
    total_villagers = Appointment.objects.filter(
        healthworker=user
    ).values('villager').distinct().count()
